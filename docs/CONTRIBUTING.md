...
> - **Edit files in `docs/`** — These are version-controlled and become the source
> - **Do NOT edit files in `publication/`** — These are auto-generated by `make docs` and `make pdf`
> - **Edit SysML source in `sysml/`** — Changes trigger regeneration of `publication/` artifacts
> - **Edit IaC in `configs/` and `fabrications/`** — These are the deployed implementations
>
> After editing `docs/` or `sysml/`, always regenerate: `make docs && make pdf`
